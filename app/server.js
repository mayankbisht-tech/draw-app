...
wss.on('connection', (ws, request) => {
  ...
  try {
    ...
    console.log(
      `[Server Debug] Connection successful to room ${roomId}. User ID: ${userId}, Name: ${userFirstName}`
    );
    ...
    const broadcastUsers = () => {
      ...
      room.forEach(client => {
        if (client.readyState === WebSocket.OPEN) {
          client.send(
            JSON.stringify({ type: 'online_users_update', users: roomUsers })
          );
        }
      });
    };
    ...
    ws.on('close', () => {
      ...
      broadcastUsers();
      console.log(
        `[Server Debug] User disconnected from room ${roomId}. Users left: ${room.size}`
      );
    });
